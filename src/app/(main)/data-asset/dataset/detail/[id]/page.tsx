...
import {
  ArrowLeft, X, Database, Eye, AlertTriangle, FileCheck,
  Sliders, Shield, Lock, Users, Globe, CheckCircle2, Layers,
  BarChart3, Tag, FileText, Image, Video, Music, Box, Brain,
  Sparkles, Cpu, BookOpen
} from "lucide-react";
...
const MOCK_DATASETS: Dataset[] = [
  ...
  // ========== 其他数据集（保留原有数据）==========
  {
    id: "mgr-001", name: "客服意图识别数据 v2.1", type: "text", source_type: "cloud", source_name: "生产环境OSS",
    item_count: 5800, quality_status: "passed", quality_score: 94.5, task_name: "意图识别v2.1",
    tags: ["意图识别", "客服", "对话AI", "已发布"],
    collaborators: ["张伟", "李娜", "王芳"],
    permission: "team",
    md5_dedup: true, import_format: ["JSONL", "CSV", "TXT"],
    split_ratio: { train: 70, val: 15, test: 15 },
    eval_score: 94.5, publish_status: "published", claimed_count: 12, download_count: 38,
    versions: [
  ...
  {
    id: "mgr-002", name: "产品分类图像数据 v1.2", type: "image", source_type: "local", source_name: "本地文件",
    item_count: 8500, quality_status: "passed", quality_score: 91.2, task_name: "商品分类",
    tags: ["图像分类", "电商", "商品识别"],
    collaborators: ["王芳"],
    permission: "private", md5_dedup: true, import_format: ["JPG", "PNG", "ZIP"],
    split_ratio: { train: 70, val: 15, test: 15 },
    eval_score: 91.2, publish_status: "published", claimed_count: 5, download_count: 18,
    versions: [
  ...
  {
    id: "mgr-003", name: "情感分析语料库 v1.0", type: "text", source_type: "platform", source_name: "平台数据",
    item_count: 12000, quality_status: "pending", quality_score: 88.7, task_name: "情感分析v1.0",
    tags: ["情感分析", "NLP", "自然语言"],
    collaborators: ["李娜", "赵磊"],
    permission: "team", md5_dedup: true, import_format: ["JSONL", "CSV"],
    eval_score: 88.7, publish_status: "draft",
    versions: [
  ...
  {
    id: "mgr-005", name: "多模态图文数据 v1.0", type: "multimodal", source_type: "public", source_name: "HuggingFace",
    item_count: 32000, quality_status: "pending", quality_score: 85.3, task_name: "多模态理解",
    tags: ["多模态", "图文对", "公开数据"],
    collaborators: ["李娜"],
    permission: "public", md5_dedup: true, import_format: ["JSONL", "Parquet"],
    eval_score: 85.3, publish_status: "draft",
    versions: [
  ...
  {
    id: "mgr-006", name: "语音指令识别集 v1.0", type: "audio", source_type: "cloud", source_name: "备份OBS",
    item_count: 4200, quality_status: "failed", task_name: "语音指令",
    tags: ["语音识别", "ASR", "指令控制"],
    collaborators: ["张伟"],
    permission: "private", md5_dedup: false, import_format: ["WAV", "MP3", "FLAC"],
    versions: [
...
function getDatasets(): Dataset[] {
  ...
  return d ? JSON.parse(d) : MOCK_DATASETS;
}

// ==================== Badges ====================
function TypeBadge({ type }: { type: string }) {
  const map: Record<string, { bg: string; color: string; label: string }> = {
    image: { bg: "#ede9fe", color: "#7c3aed", label: "图像" },
    text: { bg: "#dbeafe", color: "#1d4ed8", label: "文本" },
    video: { bg: "#fce7f3", color: "#be185d", label: "视频" },
    audio: { bg: "#fef3c7", color: "#b45309", label: "音频" },
    multimodal: { bg: "#d1fae5", color: "#047857", label: "多模态" },
    conversation: { bg: "#ede7f3", color: "#5535db", label: "对话" },
    knowledge: { bg: "#cffafe", color: "#0e7490", label: "知识图谱" },
    embedding: { bg: "#f3e8ff", color: "#6d28d9", label: "Embedding" },
    instruction: { bg: "#ffedd5", color: "#c2410c", label: "指令" },
  };
  const t = map[type] || { bg: "#f1f5f9", color: "#64748b", label: type };
  return <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 600, background: t.bg, color: t.color }}>{t.label}</span>;
}

function PermissionBadge({ perm }: { perm: string }) {
  const map: Record<string, { bg: string; color: string; label: string }> = {
    private: { bg: "#fef3c7", color: "#b45309", label: "私有" },
    team: { bg: "#dbeafe", color: "#1d4ed8", label: "团队" },
    public: { bg: "#dcfce7", color: "#15803d", label: "公开" },
  };
  const p = map[perm] || map.private;
  return <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 600, background: p.bg, color: p.color }}>{p.label}</span>;
}

function PublishStatusBadge({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string; label: string }> = {
    published: { bg: "#dcfce7", color: "#15803d", label: "已发布" },
    draft: { bg: "#fef3c7", color: "#b45309", label: "草稿" },
...
function QualityBadge({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string; label: string }> = {
    passed: { bg: "#dcfce7", color: "#15803d", label: "通过" },
    pending: { bg: "#fef9c3", color: "#ca8a04", label: "待检测" },
    failed: { bg: "#fee2e2", color: "#dc2626", label: "未通过" },
  };
  const q = map[status] || map.pending;
  return <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 600, background: q.bg, color: q.color }}>{q.label}</span>;
}

function ScoreBadge({ score }: { score?: number }) {
  if (!score) return <span style={{ color: "#cbd5e1" }}>—</span>;
  const color = score >= 90 ? "#10b981" : score >= 80 ? "#f59e0b" : "#ef4444";
  return <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 700, background: `${color}18`, color }}>{score.toFixed(1)}</span>;
}

// ==================== Tabs ====================
type TabKey = "overview" | "split" | "eval";
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 20 }}>
        {[
          ...
          { label: "协作成员", value: String(ds.collaborators?.length || 0), color: "#8b5cf6", unit: "人" },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ background: "#f8fafc", borderRadius: 10, padding: 18, border: "1px solid #e2e8f0" }}>
          ...
          {[
            ...
            <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #e2e8f0", fontSize: 12 }}>
        ...
        <div style={{ background: "#f8fafc", borderRadius: 10, padding: 18, border: "1px solid #e2e8f0" }}>
          <h4 style={{ fontSize: 13, fontWeight: 600, color: "#475569", marginBottom: 14 }}>标签</h4>
          {(!ds.tags || ds.tags.length === 0) ? (
            <div style={{ color: "#94a3b8", fontSize: 12 }}>暂无标签</div>
          ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {ds.tags.map(tag => (
                <span key={tag} style={{ padding: "3px 10px", borderRadius: 20, background: "#eff6ff", color: "#2563eb", fontSize: 11, fontWeight: 500 }}>{tag}</span>
              ))}
            </div>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[{ key: "ratio", label: "比例划分" }, { key: "rule", label: "规则划分" }].map(t => (
          <button key={t.key} onClick={() => setSplitType(t.key as "ratio" | "rule")} style={{
      ...
      {splitType === "ratio" ? (
        <>
          ...
          {splitting && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12, color: "#475569" }}>
                <span>正在划分数据...</span><span style={{ color: "#3b82f6" }}>{Math.round(progress)}%</span>
              </div>
          ...
          <button onClick={handleSplit} disabled={splitting || train + val + test !== 100} style={{
...
function EvalTab({ ds }: { ds: Dataset }) {
  ...
  const [reportGenerated, setReportGenerated] = useState(!!ds.quality_score);
        ...
        {ds.quality_score ? (
          <>
            ...
            <div style={{ marginTop: 12, height: 6, background: "#e2e8f0", borderRadius: 3 }}>
              <div style={{ height: "100%", width: `${ds.quality_score}%`, background: ds.quality_score >= 90 ? "#10b981" : ds.quality_score >= 80 ? "#f59e0b" : "#ef4444", borderRadius: 3 }} />
            </div>
          </>
        ) : (
              ...
              <div key={name}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: isAnomaly ? "#ef4444" : "#1e293b" }}>{score > 0 ? score.toFixed(1) : "—"}</span>
                    <span style={{ fontSize: 10, color: "#94a3b8" }}>/ {threshold}</span>
                  </div>
                </div>
                <div style={{ height: 4, background: "#f1f5f9", borderRadius: 2 }}>
                  <div style={{ height: "100%", width: `${Math.min((score / 100) * 100, 100)}%`, background: isAnomaly ? "#ef4444" : score >= threshold ? "#10b981" : "#f59e0b", borderRadius: 2 }} />
                </div>
              </div>
            );
          })}
      ...
      {reportGenerated && (
        <div style={{ background: "#f8fafc", borderRadius: 8, padding: "12px 16px", marginBottom: 16, border: "1px solid #e2e8f0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button style={{ padding: "4px 12px", borderRadius: 4, border: "1px solid #e2e8f0", background: "#fff", color: "#3b82f6", fontSize: 11, cursor: "pointer" }}>导出报告</button>
          </div>
        </div>
      )}

      <button onClick={handleEvaluate} disabled={evaluating} style={{
        width: "100%", padding: "10px", borderRadius: 6, border: "none",
...
export default function DatasetDetailPage() {
  ...
  useEffect(() => {
    ...
    const found = all.find((d: Dataset) => d.id === id);
    setDs(found || null);
  }, [id]);
    ...
    return (
      <main style={{ flex: 1, overflowY: "auto", padding: "24px", background: "#f8fafc", minHeight: "100vh" }}>
        <div style={{ textAlign: "center", padding: "60px 0", color: "#94a3b8" }}>
          ...
          <p style={{ fontSize: 14 }}>未找到该数据集</p>
          <button onClick={() => router.push("/data-asset/dataset")} style={{ marginTop: 16, padding: "8px 20px", borderRadius: 8, background: "#3b82f6", color: "#fff", border: "none", cursor: "pointer" }}>返回列表</button>
        </div>
      </main>
    );
  }
    ...
    <main style={{ flex: 1, overflowY: "auto", padding: "24px", background: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <button onClick={() => router.push("/data-asset/dataset")} style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b", display: "flex", alignItems: "center", gap: 4, fontSize: 13 }}>
          ...
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: "#1e293b", margin: 0 }}>{ds.name}</h1>
      ...
      {/* Tab Content */}
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", padding: 24, minHeight: 400 }}>
